...
# CORRECTED VERSION - Reference extraction moved to correct location

from datetime import datetime, date

import pandas as pd

from doc_validator.tools.reference_document_extractor import (
    add_reference_document_with_validation
)
...
def get_unique_workpacks(df: pd.DataFrame) -> list[str]:
    """
    Get list of unique workpack values from DataFrame.

    Args:
        df: Input DataFrame

    Returns:
        List of unique WP values (excluding empty/invalid)
    """
    wp_col = None
    ...
    # Get unique values, excluding empty/NA/None
    unique_wps = df[wp_col].dropna().unique()

    # Filter out invalid values
    valid_wps = []
    for wp in unique_wps:
        wp_str = str(wp).strip()
        if wp_str and wp_str.upper() not in ["N/A", "NA", "NONE", ""]:
            valid_wps.append(wp_str)

    return valid_wps


def split_dataframe_by_wp(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    """
    Split DataFrame into separate DataFrames by workpack value.

    Args:
        df: Input DataFrame with WP column

    Returns:
        Dictionary mapping WP value to DataFrame
        Example: {"A873-060126-CHK-A05": df1, "A864-050426-CHK-A09": df2}
    """
    wp_col = None
    for col in df.columns:
        if str(col).upper() == "WP":
    ...
    if wp_col is None:
        print("   ⚠️ No WP column found, cannot split")
        return {}

    workpack_dfs = {}

    # Group by WP value
    for wp_value in get_unique_workpacks(df):
        wp_df = df[df[wp_col] == wp_value].copy()
        if not wp_df.empty:
            workpack_dfs[wp_value] = wp_df
...
def process_combined_excel(
    ...
    """
    Process a combined Excel file containing multiple workpacks.

    Automatically detects workpacks by grouping rows by WP column,
    then processes each workpack separately.

    Args:
        file_path: Path to combined Excel file
        filter_start_date: Optional start date for filtering
        filter_end_date: Optional end date for filtering
        enable_action_step_control: Enable Action Step Control sheet

    Returns:
        List of output file paths (one per workpack)
    """
    print("\n" + "=" * 60)
    print("PROCESSING COMBINED EXCEL FILE")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n1. Reading combined file: {file_path}")
        df = read_input_excel(file_path)
        print(f"   ✓ Read {df.shape[0]} rows, {df.shape[1]} columns")

        # ========== STEP 2: Detect Workpacks ==========
        print(f"\n2. Detecting workpacks in file...")
        unique_wps = get_unique_workpacks(df)

        if not unique_wps:
            print("   ✗ No valid workpack values found in WP column")
            print("   Treating as single workpack file...")
            # Fall back to single file processing
            output_file = process_excel(
                file_path,
                filter_start_date,
                filter_end_date,
                enable_action_step_control
            )
            return [output_file] if output_file else []

        if len(unique_wps) == 1:
            print(f"   ℹ️ Only one workpack found: {unique_wps[0]}")
            print("   Processing as single workpack file...")
            output_file = process_excel(
                file_path,
                filter_start_date,
                filter_end_date,
        ...
        # Multiple workpacks detected
        print(f"   ✓ Found {len(unique_wps)} workpacks:")
        for i, wp in enumerate(sorted(unique_wps), 1):
            row_count = len(df[df['WP'] == wp])
            print(f"      {i}. {wp} ({row_count} rows)")
        print()

        # ========== STEP 3: Split by Workpack ==========
        print(f"3. Splitting data by workpack...")
        workpack_dfs = split_dataframe_by_wp(df)
        print(f"   ✓ Split into {len(workpack_dfs)} separate DataFrames")
        print()

        # ========== STEP 4: Process Each Workpack ==========
        print("=" * 60)
        print(f"PROCESSING {len(workpack_dfs)} WORKPACKS SEPARATELY")
        print("=" * 60)

        for idx, (wp_value, wp_df) in enumerate(sorted(workpack_dfs.items()), 1):
            print("\n" + "🔷" * 30)
            print(f"WORKPACK {idx}/{len(workpack_dfs)}: {wp_value}")
            print("🔷" * 30)

            # Create temporary file for this workpack
            import tempfile
            import os

            # Use temp directory with workpack name
            temp_dir = tempfile.gettempdir()
            temp_filename = f"temp_{sanitize_folder_name(wp_value)}_{idx}.xlsx"
            temp_path = os.path.join(temp_dir, temp_filename)

            # Write this workpack's data to temp file
            wp_df.to_excel(temp_path, index=False, engine='openpyxl')
            print(f"   Created temporary file: {temp_filename}")

            try:
                # Process this workpack
                output_file = process_excel(
                    temp_path,
                    filter_start_date,
                    filter_end_date,
                ...
                if output_file:
                    ...
                    print(f"\n   ✓ Workpack {wp_value} processed successfully")
                else:
                    print(f"\n   ✗ Failed to process workpack {wp_value}")
        ...
        print("\n" + "=" * 60)
        print("✓ COMBINED FILE PROCESSING COMPLETE")
        print("=" * 60)
        print(f"Processed: {len(output_files)}/{len(workpack_dfs)} workpacks")
        print(f"Total processing time: {overall_time:.2f} seconds")
        print(f"Average per workpack: {overall_time / len(workpack_dfs):.2f} seconds")
        print()
        ...
        print(f"\n✗ ERROR processing combined file: {str(e)}")
        import traceback
        traceback.print_exc()
        return []
...
def process_excel(
    ...
    try:
        ...
        # ========== STEP 5.5: Extract Reference Documents ==========
        print(f"\n{step_num}. Extracting reference documents...")

        # Get connection string for database access
        try:
            from doc_validator.validation.db_connector import get_connection_string
            connection_string = get_connection_string()
            print(f"   ✓ Got connection string from db_connector")
        except Exception as e:
            print(f"   ⚠️ Could not get connection string: {e}")
            print(f"   Using fallback connection string...")
            # Fallback to hardcoded connection string
            connection_string = "Driver={ODBC Driver 17 for SQL Server};Server=DESKTOP-7BI6STN;Database=AMOS-filter-validation;Trusted_Connection=yes;"

        # add_reference_document_with_validation now returns (df, summary_df)
        df, reference_summary_df = add_reference_document_with_validation(
            df,
            ref_keywords=None,  # Let it load from database
        ...
        total_errors = counts["Missing reference"]
        ...
        # Add Reference Summary sheet if we have reference data
        if not reference_summary_df.empty:
            if extra_sheets is None:
                extra_sheets = {}
            extra_sheets["Reference Summary"] = reference_summary_df
            print(f"   [REF] Added Reference Summary sheet")

        if extra_sheets:
            print(f"   [SHEETS] Extra sheets: {', '.join(extra_sheets.keys())}")
        else:
            print("   [SHEETS] No extra sheets")
