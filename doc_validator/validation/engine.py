...
def check_ref_keywords(text, seq_value=None, header_text=None, des_text=None, stripped=None, cleaned=None):
    """
    Validation function - returns 4 states now (MISSING REVISION REMOVED):
    - "Valid"              -> Has valid reference or is exempt
    ...
    1) "Missing revision" has been COMPLETELY REMOVED - revision is no longer validated
    2) "Missing reference type" has been REMOVED completely.
    3) "Missing reference" is only enforced if the header itself has a reference.
       If wo_text_action.header has NO reference at all, rows that would be
       "Missing reference" are treated as "Valid".
    4) At SEQ 9.x (9.1, 9.2, etc.), we don't check DES for ref context.
       If a row is deemed "Missing reference", we keep it that way regardless of DES.
    5) At SEQ 4.x (4.1, 4.2, etc.), STRICT FORMAT ENFORCEMENT:
       - The answer MUST follow execution patterns (PERFORMED STEP, etc.)
       - If has reference but wrong format -> "Wrong format"
    ...
    6. Check for primary reference
       ...
       - If HAS primary reference:
           * ✅ ALWAYS "Valid" (revision check removed)
    """
    ...
    # ========== STEP 4A: Check for ordering/procurement patterns (ALL SEQs including 9.x) ==========
    # These patterns indicate should be Valid for all SEQ types
    upper_text = stripped.upper()
    if 'ORDERED SPARE' or 'GET ACCESS DAMAGE AREA' or '-GET ACCESS DAMAGE AREA' in upper_text:
        return "Valid"

    # ========== STEP 4B: Execution-only answers (non-9.x SEQ) ==========
    ...
    # For SEQ 4.x, if there's a reference but no execution pattern -> "Wrong format"
    if is_seq_4x(seq_value):
        ...
        if not has_execution:
            ...
            # Check if they provided reference (wrong approach for 4.x)
            if primary:
                return "Wrong format"
    ...
    if primary:
        ...
        # ========== REMOVED: Missing revision check ==========
        # Revision validation has been completely removed
        # All references with primary keywords (AMM, SRM, CMM, etc.) are now Valid
        # regardless of whether they have revision numbers or not
    ...
    # Since we removed revision checking, this is always Valid
    return "Valid"