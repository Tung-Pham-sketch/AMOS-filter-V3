...
# ADDED: Update Conditions button to reload validation rules from database
...
# NEW: Import for updating validation rules
...
class MainWindow(QMainWindow):
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # NEW: Database connection string for validation engine
        ...
        # NEW: Initialize validation engine on startup
    ...
    # ---------------------- NEW: Validation Engine Management ----------------------

    def _initialize_validation_engine(self) -> None:
        """
        Initialize validation engine from database on startup.
        Shows success/failure message in console.
        """
        try:
            self._append_log("=" * 60 + "\n")
            self._append_log("Initializing validation engine from database...\n")
            self._append_log("=" * 60 + "\n")

            rule_manager = initialize_validation_engine(self.connection_string)

            self._append_log("\n✓ Validation engine ready\n")
            self._append_log(f"  - {len(rule_manager.ref_keywords)} reference document types\n")
            self._append_log(f"  - {len(rule_manager.iaw_keywords)} linking keywords\n")
            self._append_log(f"  - {len(rule_manager.execution_patterns)} execution patterns\n")
            self._append_log(f"  - {len(rule_manager.revision_patterns)} revision patterns\n")
            self._append_log(f"  - {len(rule_manager.seq_rules)} SEQ rules\n")
            self._append_log("=" * 60 + "\n\n")

        except Exception as e:
            self._append_log(f"\n❌ ERROR: Failed to initialize validation engine\n")
            self._append_log(f"   {str(e)}\n")
            self._append_log("   Application will use fallback hardcoded rules.\n\n")

            QMessageBox.warning(
                self,
                "Database Connection Failed",
                f"Could not connect to validation database:\n\n{str(e)}\n\n"
                f"The application will continue using fallback rules.\n"
                f"Please check that SQL Server is running."
            )

    def _on_update_conditions_clicked(self) -> None:
        """
        Reload validation rules from database.
        Called when user clicks "Update Conditions" button.
        """
        # Ask for confirmation
        reply = QMessageBox.question(
            self,
            "Update Validation Conditions",
            "This will reload all validation rules from the database.\n\n"
            "Any changes you made in the database will take effect immediately.\n\n"
            "Continue?",
        ...
        # Show in console
        self._append_log("\n" + "=" * 60 + "\n")
        self._append_log("🔄 UPDATING VALIDATION CONDITIONS...\n")
        self._append_log("=" * 60 + "\n")

        try:
            # Reinitialize the validation engine (reloads from database)
            rule_manager = initialize_validation_engine(self.connection_string)

            self._append_log("\n✓ Validation conditions updated successfully!\n")
            self._append_log(f"  - {len(rule_manager.ref_keywords)} reference document types\n")
            self._append_log(f"  - {len(rule_manager.iaw_keywords)} linking keywords\n")
            self._append_log(f"  - {len(rule_manager.execution_patterns)} execution patterns\n")
            self._append_log(f"  - {len(rule_manager.revision_patterns)} revision patterns\n")
            ...
            QMessageBox.information(
                ...
                "Validation conditions updated successfully!\n\n"
                f"Loaded:\n"
                f"  • {len(rule_manager.ref_keywords)} reference types\n"
                f"  • {len(rule_manager.iaw_keywords)} linking keywords\n"
                f"  • {len(rule_manager.execution_patterns)} execution patterns\n"
                f"  • {len(rule_manager.revision_patterns)} revision patterns\n"
                f"  • {len(rule_manager.seq_rules)} SEQ rules\n\n"
                f"New conditions will be used for the next processing run."
            )

        except Exception as e:
            self._append_log(f"\n❌ ERROR: Failed to update validation conditions\n")
            self._append_log(f"   {str(e)}\n")
            self._append_log("   Previous rules remain in effect.\n\n")

            QMessageBox.critical(
                self,
                "Update Failed",
                f"Failed to update validation conditions:\n\n{str(e)}\n\n"
                f"Previous rules remain in effect.\n\n"
                f"Please check:\n"
                f"  • SQL Server is running\n"
                f"  • Database connection is working\n"
                f"  • Database tables are accessible"
            )

        # Add this method to MainWindow class in main_window.py
        # Place it after _on_update_conditions_clicked method

        # Add this method to MainWindow class in main_window.py
        # Place it after _on_update_conditions_clicked method

    def _on_get_workpacks_clicked(self) -> None:
        """
        Extract current workpacks from database server.
        Called when user clicks "Get Current Workpacks" button.
        """
        # Show confirmation
        reply = QMessageBox.question(
            self,
            "Extract Workpacks",
            "This will extract current workpacks from the AMOS database.\n\n"
        ...
        # Show in console
        self._append_log("\n" + "=" * 60 + "\n")
        self._append_log("📥 EXTRACTING CURRENT WORKPACKS FROM DATABASE...\n")
        self._append_log("=" * 60 + "\n")

        # Disable button during extraction
        self.btn_get_workpacks.setEnabled(False)
        ...
        try:
            ...
            # Add wp_extractor.py location to path if needed
            extractor_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '..')
            if extractor_path not in sys.path:
                sys.path.insert(0, extractor_path)

            # Import and run
            self._append_log("Loading extractor module...\n")

            try:
                import wp_extractor
                self._append_log("✓ Extractor module loaded\n\n")

                # Run extraction
                wp_extractor.main()

                self._append_log("\n✓ Workpack extraction complete!\n")
                self._append_log("=" * 60 + "\n\n")

                # Refresh file list to show new file
                self._append_log("Refreshing file list...\n")
                self._load_files_from_current_source()
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    "Workpack extraction completed successfully!\n\n"
                    "The extracted file has been added to the INPUT folder.\n"
                    "You can now select it and run processing."
                )

            except ImportError as e:
                self._append_log(f"\n❌ ERROR: Could not import wp_extractor module\n")
                self._append_log(f"   {str(e)}\n")
                self._append_log("   Make sure wp_extractor.py is in the project root.\n\n")

                QMessageBox.critical(
                    self,
                    "Import Error",
                    f"Could not load extractor module:\n\n{str(e)}\n\n"
                    f"Please ensure wp_extractor.py is in the project root directory."
                )
            ...
            QMessageBox.critical(
                self,
                "Extraction Failed",
                f"Workpack extraction failed:\n\n{str(e)}\n\n"
                f"Please check:\n"
                f"  • Database connection is working\n"
                f"  • wp_extractor.py is configured correctly\n"
                f"  • wp_cur.dbf file exists"
            )

        finally:
            # Re-enable buttons
            self.btn_get_workpacks.setEnabled(True)
    ...
    def _setup_ui(self) -> None:
        ...
        # ========== NEW: UPDATE CONDITIONS BUTTON ==========
        self.btn_update_conditions = QPushButton("🔄 Update Conditions")
        self.btn_update_conditions.clicked.connect(self._on_update_conditions_clicked)
        self.btn_update_conditions.setStyleSheet("""
            QPushButton {
                font-weight: bold;
                font-size: 13px;
                padding: 10px 20px;
        ...
        """)
        self.btn_update_conditions.setToolTip(
            "Reload validation rules from database\n"
            "Use this after updating conditions in the database"
        )
        left_layout.addWidget(self.btn_update_conditions)

        # ========== NEW: GET CURRENT WORKPACKS BUTTON ==========
        self.btn_get_workpacks = QPushButton("📥 Get Current Workpacks")
        self.btn_get_workpacks.clicked.connect(self._on_get_workpacks_clicked)
        self.btn_get_workpacks.setStyleSheet("""
                    QPushButton {
                        font-weight: bold;
                        font-size: 13px;
                        padding: 10px 20px;
                ...
                """)
        self.btn_get_workpacks.setToolTip(
            "Extract current workpacks from AMOS database\n"
            "Creates a combined Excel file in the INPUT folder"
        )
        left_layout.addWidget(self.btn_get_workpacks)

        # The existing addStretch() should come after this
    ...
    def _on_run_clicked(self) -> None:
        ...
        self.worker = ProcessingWorker(
            ...
            connection_string=self.connection_string,  # Pass connection string
        )
