...
from doc_validator.core.excel_pipeline import (
    process_excel,
    process_combined_excel,
    get_unique_workpacks,
    read_input_excel,
)
...
class ProcessingWorker(QThread):
    ...
    Background worker that:
      ...
      * AUTO-DETECTS combined files (multiple WPs) and splits them
      * Runs the Excel processing pipeline (process_excel or process_combined_excel)
    ...
    def _detect_if_combined(self, local_path: str) -> bool:
        """
        Auto-detect if file contains multiple workpacks.

        Args:
            local_path: Path to Excel file

        Returns:
            True if file has multiple unique WP values, False otherwise
        """
        try:
            # Quick read to check WP column
            df = read_input_excel(local_path)
            unique_wps = get_unique_workpacks(df)

            if len(unique_wps) > 1:
                self._emit_log_and_count(
                    f"   🔍 Auto-detected: COMBINED file with {len(unique_wps)} workpacks\n"
                )
                return True
            else:
                self._emit_log_and_count(
                    f"   🔍 Auto-detected: SINGLE workpack file\n"
                )
                return False

        except Exception as e:
            self._emit_log_and_count(
                f"   ⚠️ Could not auto-detect file type: {e}\n"
                f"   Treating as single workpack file\n"
            )
            return False
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        try:
            ...
            for idx, file_info in enumerate(self.selected_files, start=1):
                ...
                # ========== AUTO-DETECT: Combined vs Single ==========
                is_combined = self._detect_if_combined(local_path)

                if is_combined:
                    # Process as combined file (multiple workpacks)
                    output_files = process_combined_excel(
                        local_path,
                    ...
                    if output_files:
                        self._emit_log_and_count(
                            f"✓ Combined file processing finished for {file_info.name}\n"
                            f"  Created {len(output_files)} output file(s)\n"
                        )
                        # Store first output file as representative
                        output_file = output_files[0] if output_files else None
                    else:
                        self._emit_log_and_count(
                            f"✗ Combined file processing failed for {file_info.name}\n"
                        )
                        output_file = None

                else:
                    # Process as single workpack file
