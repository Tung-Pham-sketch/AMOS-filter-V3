...
step_num=None) -> pd.DataFrame:
    ...
    if rule_manager:
        ref_keywords = rule_manager.ref_keywords
    else:
        ref_keywords = ['AMM', 'SRM', 'CMM', 'FIM', 'IPD', 'DMC']

    df = add_reference_document_with_validation(df, ref_keywords)
    step_num += 1
